...
import (
	...
	"git.edenfarm.id/project-version3/erp-services/erp-promotion-service/internal/app/handler"
...
func StartGrpcServer() (err error) {
	...
	// setup handler
	promotionGrpcHandler := &handler.PromotionGrpcHandler{
		Option:                  global.Setup,
		ServicesVoucher:         service.NewVoucherService(),
		ServicesVoucherItem:     service.NewVoucherItemService(),
		ServicesVoucherLog:      service.NewVoucherLogService(),
		ServicesPriceTieringLog: service.NewPriceTieringLogService(),
	}
	pb.RegisterPromotionServiceServer(srv, promotionGrpcHandler)
