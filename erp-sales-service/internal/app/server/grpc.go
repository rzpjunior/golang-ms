...
import (
	...
	"git.edenfarm.id/project-version3/erp-services/erp-sales-service/internal/app/handler"
...
func StartGrpcServer() (err error) {
	...
	// setup handler
	salesGrpcHandler := &handler.SalesGrpcHandler{
		Option:                  global.Setup,
		ServiceSalesOrder:       service.NewSalesOrderService(),
		ServicePaymentMethod:    service.NewPaymentMethodService(),
		ServicePaymentChannel:   service.NewPaymentChannelService(),
		ServicePaymentGroupComb: service.NewPaymentGroupCombService(),
		ServiceSalesInvoice:     service.NewSalesInvoiceService(),
	}
	pb.RegisterSalesServiceServer(srv, salesGrpcHandler)
