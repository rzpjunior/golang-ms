...
class debitNoteController extends Controller
{
    public function getPrint(Request $request)
    {
        ...
        $qrCode = QrCode::generate( $request['dn']['code']);
        $pdf = new Dompdf();
        ...
        $data = array(
            ...
            'recognitionDate' => date_create($request['dn']['recognition_date']),
            'recognitionDateSR' => date_create($request['dn']['supplier_return']['recognition_date']),
            'ataGR' => date_create($request['dn']['supplier_return']['good_receipt']['ata_date']),
            'session' => $request['session']
        );

        $html = view('dn',$data)->render();

        $pdf->loadHtml($html);
        $pdf->render();
//       $pdf->setOptions(['defaultFont' => 'Open Sans', 'idnemoteEnabled'=> true, 'isPhpEnabled'=>true]);
