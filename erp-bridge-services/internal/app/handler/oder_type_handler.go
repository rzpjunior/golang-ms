...
func (h *OrderTypeHandler) URLMapping(r *echo.Group) {
	h.Option = global.Setup
	h.ServicesOrderType = service.NewOrderTypeService()
...
func (h OrderTypeHandler) Get(c echo.Context) (err error) {
	...
	var total int64
	orderTypes, total, err = h.ServicesOrderType.Get(ctx.Request().Context(), page.Start, page.Limit, status, search, orderBy)
	if err != nil {
		h.Option.Common.Logger.AddMessage(log.ErrorLevel, err).Print()
		return ctx.Serve(err)
	}

	ctx.DataList(orderTypes, total, page)

	return ctx.Serve(err)
}

func (h OrderTypeHandler) Detail(c echo.Context) (err error) {
	ctx := c.(*edenlabs.Context)
	...
	orderType, err = h.ServicesOrderType.GetDetail(ctx.Request().Context(), id, "")
	if err != nil {
		h.Option.Common.Logger.AddMessage(log.ErrorLevel, err).Print()
		return ctx.Serve(err)
	}

	ctx.ResponseData = orderType
