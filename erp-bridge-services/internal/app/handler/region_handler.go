...
func (h RegionHandler) Get(c echo.Context) (err error) {
	...
	var total int64
	regions, total, err = h.ServicesRegion.Get(ctx.Request().Context(), page.Start, page.Limit, status, search, orderBy)
	if err != nil {
		h.Option.Common.Logger.AddMessage(log.ErrorLevel, err).Print()
...
func (h RegionHandler) Detail(c echo.Context) (err error) {
	...
	region, err = h.ServicesRegion.GetDetail(ctx.Request().Context(), id, "")
	if err != nil {
		h.Option.Common.Logger.AddMessage(log.ErrorLevel, err).Print()
		return ctx.Serve(err)
	}
