...
func (s *PackingOrderService) Generate(ctx context.Context, req dto.PackingOrderRequestGenerate) (err error) {
	...
	fmt.Println(">>>>>>>>>>>>1<<<<<<<<<<PPPPPPPPPP", len(salesOrders.Data))
	if err != nil {
		span.RecordError(err)
		s.opt.Logger.AddMessage(log.ErrorLevel, err)
	...
	if packingOrderExisted.ID != 0 {
		...
		for idx, salesOrder := range salesOrders.Data {
			...
			var salesOrdersDetail *bridgeService.GetSalesOrderGPListResponse
			...
			fmt.Println(idx, ">>>>>>>>>>>>99<<<<<<<<<<GetSalesOrderListGPByID", salesOrdersDetail)
			...
			if salesOrder.Status != int32(statusx.ConvertStatusName(statusx.Cancelled)) {
				for i, soi := range salesOrder.Details {
		...
		fmt.Println(">>>>>>>>>>>>2<<<<<<<<<<", len(packItemMaps))

		var itemIDs []string

		var packingOrderItems []*model.PackingOrderItem
		for _, packingItem := range packItemMaps {
		...
		fmt.Println(">>>>>>>>>>>>>>>>>>>>>>>>3", packingOrderItems)
		...
		for idx, salesOrder := range salesOrders.Data {
			fmt.Println(">>>>>>>>>>>>>>>>>>LEN", len(salesOrders.Data))
			// return
			...
			if salesOrder.Status != int32(statusx.ConvertStatusName(statusx.Cancelled)) {
				for _, soi := range salesOrder.Details {
					...
					if err != nil {
						fmt.Println("GetItemDetailByInternalId>>>", err)
						span.RecordError(err)
						s.opt.Logger.AddMessage(log.ErrorLevel, err)
						err = edenlabs.ErrorRpcNotFound("catalog", "item")
						return
...
// func (s *PackingOrderService) getSalesOrderDetails(ctx context.Context, wg *sync.WaitGroup, idx int, sopnumbe string, resultChan chan<- SalesOrderDetail) {
// 	defer wg.Done()

// 	salesOrderDetail, err := s.opt.Client.BridgeServiceGrpc.GetSalesOrderListGPByID(ctx, &bridgeService.GetSalesOrderGPListByIDRequest{
// 		Id: sopnumbe,
// 	})
