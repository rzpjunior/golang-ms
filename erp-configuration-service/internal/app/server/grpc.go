...
import (
	...
	"git.edenfarm.id/project-version3/erp-services/erp-configuration-service/internal/app/handler"
...
func StartGrpcServer() (err error) {
	...
	// setup handler
	configurationHandler := &handler.ConfigurationGrpcHandler{
		Option:               global.Setup,
		ServiceCodeGenerator: service.NewCodeGeneratorService(),
		ServiceGlossary:      service.NewGlossaryService(),
		ServiceWrt:           service.NewWrtService(),
		ServiceConfigApp:     service.NewApplicationConfigService(),
		ServiceRegionPolicy:  service.NewRegionPolicyService(),
		ServiceDayOff:        service.NewDayOffService(),
	}
	pb.RegisterConfigurationServiceServer(srv, configurationHandler)
