...
// updatePriceRequestDeliveredSO : struct to hold sales order request data
type updatePriceRequestDeliveredSO struct {
	ID                     int64     `json:"-"`
...
func (r *updatePriceRequestDeliveredSO) Validate() *validation.Output {
	...
	var (
		...
		totalChargeDifferences                                                                                  float64
	...
	if r.SalesOrder.Status != 8 {
		o.Failure("id.invalid", util.ErrorStatusDoc("sales order", "updated", "sales order"))
		return o
	}
	...
	if orderTypeID == 6 {
		...
		if warehouseSelfPickUp.ID != r.Warehouse.ID {
				...
				if productID, err = common.Decrypt(v.ProductID); err == nil {
					if v.Product, err = repository.ValidProduct(productID); err == nil {
						...
						if _, countSOI, err := repository.CheckSalesOrderItemData(filtersoi, excludesoi); err == nil && countSOI == 0 {
							...
							if r.OrderType.Name == "Zero Waste" {
								filter = map[string]interface{}{"product_id": productID, "warehouse_id": r.Warehouse.ID}
							} else {
						...
						v.Subtotal = v.Quantity*float64(v.UnitPrice) - v.UnitPriceDiscount
...
func (r *updatePriceRequestDeliveredSO) Messages() map[string]string {
