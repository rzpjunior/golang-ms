...
func (h *Handler) URLMapping(r *echo.Group) {
	r.GET("", h.read, auth.Authorized("ppt_rdl"))
	r.GET("/:id", h.detail, auth.Authorized("ppt_rdd"))
	r.GET("/filter", h.readFilter, auth.Authorized("filter_rdl"))
	r.POST("", h.create, auth.Authorized("ppt_crt"))
	r.PUT("/archive/:id", h.archive, auth.Authorized("ppt_arc"))
	r.PUT("/unarchive/:id", h.unarchive, auth.Authorized("ppt_urc"))
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	var data []*model.PurchaseTerm
	var total int64

	if data, total, e = repository.GetPurchaseTerms(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) readFilter(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetFilterPurchaseTerms(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetPurchaseTerm("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
