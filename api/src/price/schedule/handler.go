...
func (h *Handler) URLMapping(r *echo.Group) {
	...
	r.GET("/:id", h.detail, auth.Authorized("prd_prc_rdl"))
	r.POST("", h.create, auth.Authorized("prd_prc_upd"))
	r.PUT("/cancel/:id", h.cancel, auth.Authorized("prd_prc_upd"))
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	var data []*model.PriceSchedule
	var total int64

	if data, total, e = repository.GetPriceSchedules(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetPriceSchedule("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
