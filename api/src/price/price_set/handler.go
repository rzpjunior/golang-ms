...
func (h *Handler) URLMapping(r *echo.Group) {
	r.GET("", h.read, auth.Authorized("prs_rdl"))
	r.GET("/:id", h.detail, auth.Authorized("filter_rdl"))
	r.GET("/filter", h.readFilter, auth.Authorized("filter_rdl"))
	r.POST("", h.create, auth.Authorized("prs_crt"))
	r.PUT("/:id", h.update, auth.Authorized("prs_upd"))
	r.PUT("/archive/:id", h.archive, auth.Authorized("prs_arc"))
	r.PUT("/unarchive/:id", h.unarchive, auth.Authorized("prs_urc"))
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	var data []*model.PriceSet
	var total int64

	if data, total, e = repository.GetPriceSets(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) readFilter(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetFilterPriceSets(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetPriceSet("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
