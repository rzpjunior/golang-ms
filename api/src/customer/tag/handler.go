...
func (h *Handler) URLMapping(r *echo.Group) {
	r.GET("", h.read, auth.Authorized("cst_tag_rdl"))
	r.GET("/:id", h.detail, auth.Authorized("cst_tag_rdd"))
	r.GET("/filter", h.readFilter, auth.Authorized("filter_rdl"))
	r.POST("", h.create, auth.Authorized("cst_tag_crt"))
	r.PUT("/archive/:id", h.archive, auth.Authorized("cst_tag_arc"))
	r.PUT("/unarchive/:id", h.unarchive, auth.Authorized("cst_tag_urc"))
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	var data []*model.TagCustomer
	var total int64

	if data, total, e = repository.GetTags(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) readFilter(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetFilterTags(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetTag("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
