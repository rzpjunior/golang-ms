...
// createRequest : struct to hold customer distribution network request data
...
// Validate : function to validate customer distribution network request data
func (c *createRequest) Validate() *validation.Output {
	...
	if c.ProspectCustomerID != "" {
		...
		if e := c.ProspectCustomer.Read("ID"); e != nil {
			o.Failure("prospect_customer_id.invalid", util.ErrorInvalidData("prospect customer"))
		}
		if c.ProspectCustomer.SalespersonID != 0 {
			if e := r.Raw("SELECT * FROM staff where id = ?", c.BranchSalesPerson.ID).QueryRow(&c.Staff); e != nil {
				o.Failure("salesperson_id.invalid", util.ErrorInvalidData("salesperson"))
			}
	...
	if c.NewMerchantCheck == "true" {
		...
		if c.MerchantPassword, err = common.PasswordHasher("12345678"); err != nil {
		...
		if c.MerchantID != "" {
			...
			if err = c.Merchant.Read("ID"); err == nil {
				if c.ProspectCustomerID != "" && c.Merchant.UpgradeStatus == 1 {
					...
					c.Merchant.Password = c.MerchantPassword
