...
// Validate : function to validate update customer distribution network request data
func (c *updateRequest) Validate() *validation.Output {
	var e error
	var creditLimitAmountDifferences float64 = 0
	...
	isNewCreditLimitAmountMoreThanPrevious := c.CreditLimitAmount > c.Merchant.CreditLimitAmount
	isLessCreditLimitAmountMoreThanPrevious := c.CreditLimitAmount < c.Merchant.CreditLimitAmount

	if isNewCreditLimitAmountMoreThanPrevious {
		creditLimitAmountDifferences = c.CreditLimitAmount - c.Merchant.CreditLimitAmount
		c.CreditLimitAfter = c.CreditLimitBefore + creditLimitAmountDifferences
		c.Merchant.RemainingCreditLimitAmount = c.CreditLimitAfter
	}

	if isLessCreditLimitAmountMoreThanPrevious {
		creditLimitAmountDifferences = c.Merchant.CreditLimitAmount - c.CreditLimitAmount
		c.CreditLimitAfter = c.CreditLimitBefore - creditLimitAmountDifferences
		c.Merchant.RemainingCreditLimitAmount = c.CreditLimitAfter
	}

	if c.CustomCreditLimit != 1 && c.Merchant.CreditLimit == nil {
		c.CreditLimitAmount = 0
		c.CreditLimitAfter = 0
		c.Merchant.RemainingCreditLimitAmount = 0

		remainingTotalCharge, e := repository.GetTotalRemainingBySOAndSI(c.Merchant.ID)
		if e != nil {
			o.Failure("merchant.invalid", util.ErrorInvalidData("merchant"))
		}

		if remainingTotalCharge > 0 {
			c.CreditLimitAfter = -remainingTotalCharge
			c.Merchant.RemainingCreditLimitAmount = -remainingTotalCharge
		}
	}

	if c.Merchant.CreditLimitAmount == 0 {
		remainingTotalCharge, e := repository.GetTotalRemainingBySOAndSI(c.Merchant.ID)
		if e != nil {
			o.Failure("merchant.invalid", util.ErrorInvalidData("merchant"))
		}
	...
	if c.CreditLimitAmount > 0 || c.Merchant.CreditLimitAmount > 0 {
