...
func (h *Handler) URLMapping(r *echo.Group) {
	r.GET("", h.read, auth.Authorized("wrt_rdl"))
	r.GET("/:id", h.detail, auth.Authorized("wrt_rdd"))
	r.GET("/filter", h.readFilter, auth.Authorized("filter_rdl"))
	r.POST("", h.create, auth.Authorized("wrt_crt"))
	r.PUT("/archive/:id", h.archive, auth.Authorized("wrt_arc"))
	r.PUT("/unarchive/:id", h.unarchive, auth.Authorized("wrt_urc"))

	r.GET("/mobile", h.read, auth.AuthorizedMobile())
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetWrts(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) readFilter(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetFilterWrts(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetWrt("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
