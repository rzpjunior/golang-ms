...
func (h *Handler) URLMapping(r *echo.Group) {
	...
	r.GET("/delivery", h.getAreaBusinessPolicyDelivery, auth.Authorized("filter_rdl"))
}
...
func (h *Handler) read(c echo.Context) (e error) {
	...
	var data []*model.AreaBusinessPolicy
	var total int64

	if data, total, e = repository.GetAreaBusinessPolicies(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) readFilter(c echo.Context) (e error) {
	...
	if data, total, e = repository.GetFilterAreaBusinessPolicies(rq); e == nil {
		ctx.Data(data, total)
	}
...
func (h *Handler) detail(c echo.Context) (e error) {
	...
	if id, e = ctx.Decrypt("id"); e == nil {
		if ctx.ResponseData, e = repository.GetAreaBusinessPolicy("id", id); e != nil {
			e = echo.ErrNotFound
		}
	}
...
func (h *Handler) getAreaBusinessPolicyDelivery(c echo.Context) (e error) {
	ctx := c.(*cuxs.Context)

	areaID, _ := common.Decrypt(ctx.QueryParam("area_id"))
	businessTypeID, _ := common.Decrypt(ctx.QueryParam("business_type_id"))

	ctx.ResponseData, e = repository.GetAreaBusinessPolicyDelivery(areaID, businessTypeID)
