...
func NewLog(serviceName string, clientLog *logrus.Logger) *Log {
	d := new(Log)
	d.Logger = log.NewLoggerWithClient(serviceName, clientLog).Logger()
...
func debugLogQueies(alias *alias, operaton, query string, t time.Time, err error, args ...interface{}) {
	...
	fullQuery := fmt.Sprintf(strings.Replace(query, "?", "'%v'", -1), args...)

	go func() {
		if flag == "FAIL" {
			DebugLog.WithFields(logrus.Fields{
				"service":  DebugLog.ServiceName,
				"alias":    alias.Name,
				"status":   flag,
				"operaton": operaton,
				"latency":  fmt.Sprintf("%.2f ms", elsp),
				"query":    fullQuery,
				"error":    err.Error(),
			}).Errorln("ORM FAIL")
		} else {
