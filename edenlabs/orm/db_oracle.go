...
import (
	...
	"git.edenfarm.id/edenlabs/edenlabs/orm/hints"
	...
	default:
		go func() {
			DebugLog.WithFields(logrus.Fields{
				"error":  "Not a valid specifying action, so that action is ignored",
				"status": "FAIL",
			}).Warnln("GenerateSpecifyIndex")
		}()
		...
		if err == nil {
			...
			if err != nil {
				go func() {
					...
					DebugLog.WithFields(logrus.Fields{
						"error":  fmt.Sprintf(ErrLastInsertIdUnavailable.Error(), ':', err),
						"status": "FAIL",
					}).Errorln("InsertValue")
				}()
