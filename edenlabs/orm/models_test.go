...
type Post struct {
	...
	Tags             []*Tag    `orm:"rel(m2m);rel_through(git.edenfarm.id/edenlabs/edenlabs/orm.PostTags)"`
}
...
type Permission struct {
	ID     int `orm:"column(id)"`
	Name   string
	Groups []*Group `orm:"rel(m2m);rel_through(git.edenfarm.id/edenlabs/edenlabs/orm.GroupPermissions)"`
}
...
var helpinfo = `need driver and source!
	...
	go get -u git.edenfarm.id/edenlabs/edenlabs/orm
	...
	go test -v git.edenfarm.id/edenlabs/edenlabs/orm
